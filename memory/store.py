...
def _schema_items_arrow_schema(dim: int = None) -> pa.Schema:
    if dim is None:
...
def _query_history_arrow_schema(dim: int = None) -> pa.Schema:
    if dim is None:
        dim = default_dimension()
    return pa.schema(
        [
...
class MemoryStore:
    ...
    def __init__(
        ...
        self._embed_fn = get_embedding_function(model_name=model_name)
